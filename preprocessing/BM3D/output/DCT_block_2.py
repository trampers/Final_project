...
def DCT(img):
    ...
    '''
    img = Trans(img)
    Freq_tmp = dct(img,norm = 'ortho')
    Freq_tmp = Trans(Freq_tmp)
    Freq_proc = dct(Freq_tmp, norm='ortho')
    return Freq_proc
...
def IDCT(Freq):
    ...
    '''
    Freq = Trans(Freq)
    img_tmp = idct(Freq, norm= 'ortho')
    img_tmp = Trans(img_tmp)
    img_result = idct(img_tmp, norm= 'ortho')
    return img_result
